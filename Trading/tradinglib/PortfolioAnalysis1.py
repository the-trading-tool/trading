...
class PortfolioAnalysis:
    def __init__(self, transactions_file, index_symbol):
        """
    ...
    def _load_data(self):
        ...
        # Download the asset data from yfinance
        self.asset_data = {}
        for asset in assets:
            self.asset_data[asset] = md.download(asset, start=start_date, end=end_date)
            self.asset_data[asset] = self.asset_data[asset].reindex(pd.date_range(start=start_date, end=end_date, freq='B')).fillna(method='ffill')
        
        # Download the index data from yfinance
        self.index_df = md.download(self.index_symbol, start=start_date, end=end_date)
        self.index_df = self.index_df.reindex(pd.date_range(start=start_date, end=end_date, freq='B')).fillna(method='ffill')
    ...
    def _calculate_index_performance(self, all_dates):
        ...
        self.index_performance = (self.index_df['Close'] / initial_value).reindex(all_dates).fillna(method='ffill')
